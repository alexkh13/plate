...
interface LidBasalRateChange extends BaseEvent {
...
export class TConnectClient {
  ...
  public async pump_events(tconnectDeviceId: string, startDate: Date, endDate: Date): Promise<(BaseEvent | null)[]> {
    ...
    try {
      ...
      const tempDir = '/data/data/com.termux/files/home/.gemini/tmp/6a9ec40eaef787ba99c26dd471a8d2a3b4bb6841748bd4c039ad2d6950dae634';
      fs.writeFileSync(`${tempDir}/tandem_raw_response.bin`, response.data);
      console.log(`Raw response saved to ${tempDir}/tandem_raw_response.bin`);
...
async function main() {
    ...
    const EMAIL = process.env.TCONNECT_EMAIL || 'alexkh13@gmail.com';
    const PASSWORD = process.env.TCONNECT_PASSWORD || '!Alx11tandem';

    if (EMAIL === 'your-email@example.com') {
        console.log('Please set TCONNECT_EMAIL and TCONNECT_PASSWORD env vars or edit the script.');
        return;
    }

    const client = new TConnectClient(EMAIL, PASSWORD, 'EU');

    try {
        console.log('Authenticating...');
        await client.login();
        console.log(`Pumper ID: ${client.pumperId}`);
        ...
        console.log(`Fetching data from ${start.toDateString()} to ${end.toDateString()}...`);

        // 3. Fetch Data
        const metadata = await client.pump_event_metadata();
        console.log('Pump event metadata:', metadata);
        if (metadata && metadata.length > 0) {
            const tconnectDeviceId = metadata[0].tconnectDeviceId;
            const events = await client.pump_events(tconnectDeviceId, start, end);
            console.log('Pump events (excluding LID_BASAL_RATE_CHANGE):');
            events
                .filter(event => event && event.name !== 'LID_BASAL_RATE_CHANGE')
                .forEach(event => {
                    if (event) {
